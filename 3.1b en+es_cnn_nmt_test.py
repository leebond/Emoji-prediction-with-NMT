...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 21:15:00 2019
...
if __name__ == '__main__':
    ...
    input_dictionary = pickle.load(open('./static/en+es_nmt_dictionary.pkl', 'rb'))
    ...
    es_cnn_model = load_model('./static/en+es_cnn_nmt_model.h5')
    p_v = es_cnn_model.predict(X_test_padded)
    pred = [np.argmax(p) for p in p_v]
       
    #create a submission
    with open('./submission/en+es_cnn_nmt_test_submission', 'w+') as submission_file:
        for p in pred:
