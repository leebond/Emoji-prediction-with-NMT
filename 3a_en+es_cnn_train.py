...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 13:04:04 2019
...
def word_for_word_translation(es_texts_subset):
    with open("./static/en-es.txt") as f:
        en_es = [l.strip() for l in f]
        en_es = [l.split() for l in en_es]
    ...
    text = []
    cnt = 0
    for sent in es_texts_subset:
        mysent = []
        for word in sent:
            for dword in en_es:
                if word == dword[1]:
                    mysent.append(dword[0])
                    break
...
if __name__ == '__main__':
    ...
    es_labeltexts_subset = list(zip(es_labels_mapped_subset, es_texts_subset))
#    with open('./static/es_labeltexts_subset.pkl', 'wb') as f:
#        pickle.dump(es_labeltexts_subset, f)
#    es_texts_subset_translated = word_for_word_translation(es_texts_subset)
#    with open('./static/es_to_en_text.pkl', 'wb') as f:
#        pickle.dump(es_texts_subset_translated, f)
    es_texts_subset_translated = pickle.load(open('./static/es_to_en_text.pkl', 'rb'))
    ...
    cnn_model.save('./static/en+es_cnn_model.h5')
    cnn_model = load_model('./static/en+es_cnn_model.h5')
    p_v = cnn_model.predict(X_train_padded)
    pred = [np.argmax(p) for p in p_v]
       
    #create a submission
    with open('./submission/en+es_cnn_submission', 'w+') as submission_file:
        for p in pred:
            submission_file.write(str(p)+'\n')
    
    with open('./static/en+es_dictionary' + '.pkl', 'wb') as f:
